...
import { type ProcessedWeather } from "../utils/weatherProcessor";
import { WEATHER_CONFIG } from "../config/weatherConfig";
import type { WeatherResponse } from "../types/WeatherResponse";
//  pure day time filtering function


export function renderMetadata(data: WeatherResponse) {
...
export function renderWeatherForecast(daytimeForecast: ProcessedWeather[], hoursToShow: number) {
  ...
  uiElements.list.innerHTML = currentDisplayData.map(({ dateObj, temp, unit }) => {
    ...
    let bgStyle = "bg-blue-900/40 border-blue-800/50 hover:bg-blue-800/60";
